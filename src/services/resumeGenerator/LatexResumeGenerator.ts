...
export class LatexResumeGenerator implements IResumeGenerator {
  ...
  async generate(
    ...
    try {
      ...
      const tmpDir = tmpdir()
      const texFile = join(tmpDir, `resume-${Date.now()}.tex`)
      const pdfFile = join(tmpDir, `resume-${Date.now()}.pdf`)
      ...
      try {
        ...
        try {
          ...
          await unlink(join(tmpDir, `resume-${Date.now()}.log`)).catch(() => {})
          await unlink(join(tmpDir, `resume-${Date.now()}.aux`)).catch(() => {})
        } catch {
