...
import { WorkExperiencesArraySchema } from "@resume-generator/shared-types";
// ESM: use import.meta.dirname instead of __dirname
const __dirname = import.meta.dirname ?? process.cwd();
const DATA_DIR = path.join(__dirname, "..", "..", "data", "work_experiences");
class WorkExperienceRepository {
    static instance;
    cachedWorkExperiences = null;
    constructor() { }
    static getInstance() {
    ...
    async getAllWorkExperiences() {
        ...
        const experiencesWithContent = await Promise.all(experiences.map(async (experience) => this.loadExperienceContent(experience)));
     ...
     */
    async loadExperienceContent(experience) {
    ...
    async getAllCategories() {
        ...
        experiences.forEach((e) => e.categories.forEach((c) => categorySet.add(c.category_name)));
        return Array.from(categorySet).sort();
    }
    /**
     * Get work experiences by IDs (preserves order)
     */
    async getWorkExperiencesByIds(ids) {
        const experiences = await this.getAllWorkExperiences();
     ...
     */
    async updateWorkExperienceNote(experienceId, note) {
        const experiencesPath = path.join(DATA_DIR, "work_experiences.json");
        // Read the raw JSON
        const experiencesJson = await fs.readFile(experiencesPath, "utf-8");
        const rawExperiences = JSON.parse(experiencesJson);
        // Find the experience by ID
        const experienceIndex = rawExperiences.findIndex((e) => e.id === experienceId);
     ...
     */
    async updateWorkExperienceArchived(experienceId, archived) {
        const experiencesPath = path.join(DATA_DIR, "work_experiences.json");
        // Read the raw JSON
        const experiencesJson = await fs.readFile(experiencesPath, "utf-8");
        const rawExperiences = JSON.parse(experiencesJson);
    ...
    async createWorkExperience(data) {
        ...
        let counter = 1;
        // Ensure ID is unique
        while (rawExperiences.some((e) => e.id === id)) {
            id = `${baseId}-${counter}`;
            counter++;
        }
    ...
    async updateWorkExperience(experienceId, data) {
        ...
        if (data.categories) {
            ...
            for (const category of data.categories) {
                ...
                const existingCategory = experience.categories.find((c) => this.slugify(c.category_name) === categorySlug);
