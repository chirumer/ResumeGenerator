...
import { getWorkExperienceForResume, getProjectsForResume, } from '../services/dataExtractor.js';
 ...
 */
async function calculateVerticalSpace(latexContent, targetPages) {
...
\\makeatother
`;
    const instrumentedLatex = latexContent.replace('\\end{document}', probe + '\n\\end{document}');
...
app.post('/', zValidator('json', VerticalSpaceRequestSchema), async (c) => {
    ...
    try {
        ...
        const workExperiences = await getWorkExperienceForResume(data.workExperienceIds, data.workExperienceCategories, { workExperienceRepository });
        // Extract data for projects
        const projects = await getProjectsForResume(data.projectIds, data.projectCategories, { projectRepository });
