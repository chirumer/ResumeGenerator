...
app.get('/', async (c) => {
    try {
        ...
        return c.json({ error: error instanceof Error ? error.message : 'Failed to fetch general information' }, 500);
    }
});
...
app.post('/', zValidator('json', UpdateGeneralSchema), async (c) => {
    ...
    try {
        ...
        const updated = await generalRepository.updateGeneralInfo(data);
        return c.json({ success: true, data: updated });
    }
    catch (error) {
        return c.json({ error: error instanceof Error ? error.message : 'Failed to update general information' }, 500);
    }
});
