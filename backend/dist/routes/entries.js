...
app.post('/', zValidator('json', EntriesRequestSchema), async (c) => {
    ...
    try {
        ...
        if (operation !== 'create' && !entryId) {
            return c.json({ error: 'entryId is required for update, archive, and delete operations' }, 400);
        }
        // Validate that data is provided for create and update operations
        if ((operation === 'create' || operation === 'update') && !data) {
            return c.json({ error: 'data is required for create and update operations' }, 400);
        }
        // Validate data based on section
        if (data && section === 'projects') {
            const validationResult = ProjectDataSchema.safeParse(data);
            if (!validationResult.success) {
                return c.json({ error: 'Invalid project data', details: validationResult.error.flatten() }, 400);
            }
        }
        else if (data && section === 'work-experiences') {
            const validationResult = WorkExperienceDataSchema.safeParse(data);
            if (!validationResult.success) {
                return c.json({ error: 'Invalid work experience data', details: validationResult.error.flatten() }, 400);
            }
        }
        ...
        return c.json({ error: errorMessage, details: error instanceof Error ? error.stack : undefined }, 500);
    }
});
